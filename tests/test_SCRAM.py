...
def test_SCRAM(testNFT, rentable, paymentToken, operator, weth, orentable, wrentable, governance, accounts, chain):
    ...
    rentable.createOrUpdateLeaseConditions(
        testNFT, tokenId+1, paymentToken, maxTimeDuration, pricePerBlock, {"from": user}
    )
    ...
    if (paymentToken == weth.address):
        ...
        rentable.createLease(testNFT, tokenId, subscriptionDuration, {"from": subscriber})
    elif (paymentToken == "0x0000000000000000000000000000000000000000"):
        rentable.createLease(testNFT, tokenId, subscriptionDuration, {"from": subscriber, "value": value})
    ...
    rentable.emergencyExecute(testNFT, 0, testNFT.transferFrom.encode_input(rentable, governance, tokenId), False, 200000, {"from": governance})
    ...
    rentable.emergencyBatchWithdrawERC721(testNFT, [tokenId, tokenId+1], True, {"from": governance})
    assert testNFT.ownerOf(tokenId) == governance.address
    ...
    if (paymentToken == weth.address):
        ...
        rentable.emergencyWithdrawERC20ETH("0x0000000000000000000000000000000000000000", {"from": governance})
